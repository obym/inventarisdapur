          ...
          <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
            <div>
              {editItem && onDelete && (
                <button
                  type="button"
                  id="btn-delete-from-form"
                  onClick={() => {
                    onClose();
                    onDelete(editItem.id);
                  }}
                  className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-rose-600 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded-lg transition cursor-pointer"
                >
                  <Trash2 className="w-3.5 h-3.5" />
                  <span>Hapus Barang Ini</span>
                </button>
              )}
            </div>
